...
class DBMgr{
	...
	public	$DBMgr_init_status	=	true;
	public	$DBMgr_init_error	=	'';
	...
	private function connect_SpecificDatabase(){
		...
		if (!$db) {
			$this->DBMgr_init_status	=	false;
			$this->DBMgr_init_error		=	"Error: Connection failed '".mysqli_connect_error()."'";
		}
...
*/
	...
	/*
	Description: Returns an indexed array of table names present in the database
	*/
	public function DB_Tables(){
		$output	=	array();

		$Query	=	"SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_TYPE='BASE TABLE' AND TABLE_SCHEMA='".trim($this->DatabaseName)."'";
		$tableNames	=	$this->Query($Query, 'NUM_ARR');
		for($i = 0; $i< count($tableNames); $i++){
			$output[]	=	$tableNames[$i][0];
		}
